...
<body>
<?= form_open_multipart('konten/update'); ?>
<?php if(validation_errors()): ?>
    <div class="validation-errors"><?= validation_errors(); ?></div>
<?php endif; ?>

    <input type="hidden" name="id_konten" value="<?= $this->uri->segment(3,0)?>">
    <input type="hidden" name="nama_foto" value="<?= $kontenn->foto?>">
        <label for="">Judul</label>

        <input type="text" name="judul" placeholder="masukkan username" value="<?= $kontenn->judul?>">
    ...
    <select name="id_kategori" id="">
        ...
        <textarea name="isi_konten" id="" cols="30" rows="10">
            <?= $kontenn->isi_konten?>
        </textarea>

        <label for="">Tanggal</label>
 
        <input type="date" name="tanggal" placeholder="masukkan nama " value="<?= $kontenn->tanggal?>">
  
        <label for="">Foto</label>
 
        <input type="file" name="foto" value="<?= $kontenn->foto?>" accept="image/png, image/jpg, image/jpeg">
 
        <label for="">Audio</label>
    
        <input type="text" name="audio" id="audio" value="<?= $kontenn->audio?>">

        <label for="">OST</label>
    
    <input type="text" name="ost" id="ost" value="<?= $kontenn->ost?>">
